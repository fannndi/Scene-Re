    ...
    private fun drawCycle(canvas: Canvas) {
        ...
        */
        ...
        if (temperature >= 48 || ratioState < 11) {
            ...
            cyclePaint!!.color = ContextCompat.getColor(context, R.color.colorAccent)
        } else {
            cyclePaint!!.color = ContextCompat.getColor(context, R.color.color_load_none)
        }
        ...
        val mSweepGradient = SweepGradient(
            canvas.getWidth() / 2f,
            canvas.getHeight() / 2f, // use the arc center as the sweep gradient center to achieve the desired effect
            intArrayOf(
