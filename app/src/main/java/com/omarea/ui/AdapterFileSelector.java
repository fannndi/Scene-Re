...
public class AdapterFileSelector extends BaseAdapter {
    ...
    private boolean hasParent = false; // whether there is a parent directory
    private String rootDir = "/"; // root directory
    private boolean leaveRootDir = true; // whether leaving the configured rootDir for parent directories is allowed
    private boolean folderChooserMode = false; // whether in folder chooser mode (files are hidden and long-press selects a folder)
        ...
        if (hasParent && position == 0) {
            ...
            if (file.isDirectory()) {
                ...
                view.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        if (!file.exists()) {
                            Toast.makeText(view.getContext(), "Selected file has been deleted. Please choose again!", Toast.LENGTH_SHORT).show();
                            return;
                        }
                        ...
                        if (files != null && files.length > 0) {
                            ...
                            Snackbar.make(view, "No files in this directory!", Snackbar.LENGTH_SHORT).show();
                        }
                    }
                });
                if (folderChooserMode) {
                        ...
                        public boolean onLongClick(View v) {
                            DialogHelper.Companion.confirm(view.getContext(),
                                    ...
                                    new Runnable() {
                                        ...
                                        public void run() {
                                            if (!file.exists()) {
                                                Toast.makeText(view.getContext(), "Selected folder has been deleted. Please choose again!", Toast.LENGTH_SHORT).show();
                                                return;
                                            }
                                            selectedFile = file;
                                            fileSelected.run();
