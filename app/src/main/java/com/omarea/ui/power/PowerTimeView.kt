...
class PowerTimeView : View {
    ...
    private var ladder = false // ladder mode
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val maxTimeMinutes = if (startTime != null && maxTime != null) {
            ((maxTime - startTime) / 60000.0).toDouble()
        } else {
            30.0
        }
            ...
            */
            ...
            if (ladder) {
                for (sample in samples) {
                    // If the gap between samples is too long (fill in fake data: dashed line)
                    if ((sample.startTime - lastSample.endTime) > 10000) {
                        ...
                        // Highlight samples while the screen is on, otherwise grey
                ...
                for (sample in samples) {
                    val currentX = ((sample.startTime - startTime) / 60000f * ratioX).toFloat() + innerPadding
                    val currentY = startY - (sample.capacity * ratioY)
                    // If the gap between samples is too long
                    if ((sample.startTime - lastSample.endTime) > 10000) {

                        // First draw a line to the previous sample's endTime
                        paint.pathEffect = null
                        ...
                        // Then fill in fake data (dashed line)
                        paint.pathEffect = dashPathEffect
                        // Highlight samples while the screen is on, otherwise grey
                        if (lastSample.screenOn) {
                            paint.color = Color.parseColor("#1474e4")
