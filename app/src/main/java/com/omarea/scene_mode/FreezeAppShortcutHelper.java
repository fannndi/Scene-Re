...
public class FreezeAppShortcutHelper {
    ...
    /**
     * Remove the shortcut (in practice this does not work on any Android version).
     *
     * @param context
     * @param packageName
     * @return
    ...
    public ArrayList<String> getPinnedShortcuts(Context context) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            // in practice the pinned shortcut data can basically never be read
    ...
    public boolean createShortcut(Context context, String packageName) {
        ...
        try {
            ...
            // shortcut name
            shortcut.putExtra(Intent.EXTRA_SHORTCUT_NAME, prefix + applicationInfo.loadLabel(packageManager));// shortcut name
            shortcut.putExtra("duplicate", false); // whether duplicates may be created
        ...
        try {
            ...
            if (shortcutManager.isRequestPinShortcutSupported()) {
                ...
                final ShortcutInfo info = new ShortcutInfo.Builder(context, packageName)
                        ...
                        .setActivity(new ComponentName(context, ActivityQuickStart.class)) // only the "main" activity - the one declaring the Intent#ACTION_MAIN and Intent#CATEGORY_LAUNCHER intent filters - can be the target activity
                        .build();

                // called back when the add-shortcut confirmation dialog appears
