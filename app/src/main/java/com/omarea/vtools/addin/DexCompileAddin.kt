...
class DexCompileAddin(private var context: ActivityBase) : AddinBase(context) {
    ...
    fun modifyConfig() {
        // The `SDK_INT < N` fallback into modifyConfigOld() was unreachable:
        // minSdk is 29, and modifyConfigOld() only rewrote the removed
        // dalvik.vm.dex2oat-filter build.props properties. Removed with it.
        val arr = arrayOf(
                "No compile (faster install)",
                "Compile (faster runtime)",
                "Restore default")
        val installMode = PropsUtils.getProp("pm.dexopt.install")
        var index = when (installMode) {
            "extract",
            "quicken",
            "interpret-only",
            "verify-none" -> 0
            "speed",
            "everything" -> 1
            else -> if (PropsUtils.getProp("pm.dexopt.core-app") == "verify-none") 3 else 0
        }
