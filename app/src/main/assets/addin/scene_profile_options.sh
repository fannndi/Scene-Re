...
# +---------------------------------------------------------------+
# | Stock MIUI IRQ balancer (opt-in, adopted configuration)        |
# +---------------------------------------------------------------+
#
# The ROM ships /vendor/bin/msm_irqbalance with three confs but keeps all
# three init services `disabled`. Adapted from the IRQ-Balancer-Configuration
# module (LeanxModulostk/IRQ-Balancer-Configuration, MIT-style public module):
# the graphics and timer IRQs are discovered from /proc/interrupts and
#   - excluded from the balancer (IGNORED_IRQ), and
#   - pinned explicitly (msm_drm/sde -> little cores 0-2, kgsl-3d0 -> 1-2),
# while PRIO=1,1,1,0,0,0,0,0 keeps the balanced IRQs off the big cluster.
# Scene runs the stock binary with its own generated conf; the affinities are
# snapshotted and re-asserted on every apply (no bind-mount tricks), and the
# instance is stopped again when the toggle goes off.
IRQ_CONF="/data/adb/scene/irqbalance.conf"
IRQ_IGNORE_PATTERN="arch_timer|arm-pmu|arch_mem_timer|kgsl|msm_drm|sde"
IRQ_KGSL_PATTERN="kgsl"
IRQ_KGSL_AFFINITY="1-2"
IRQ_DRM_PATTERN="msm_drm|sde"
IRQ_DRM_AFFINITY="0-2"

irq_numbers_for() {
    grep -E "$1" /proc/interrupts 2> /dev/null | awk -F: '{print $1}' | tr -d ' \t'
}

irq_write_conf() {
    local ignored
    ignored="$(irq_numbers_for "$IRQ_IGNORE_PATTERN" | tr '\n' ',' | sed 's/,$//')"
    {
        echo "# Generated by Scene - stock msm_irqbalance with MIUI's graphics/timer"
        echo "# IRQs excluded; those are pinned by the options layer and must not be"
        echo "# moved back. PRIO favours the little cluster."
        echo "PRIO=1,1,1,0,0,0,0,0"
        echo "IGNORED_IRQ=$ignored"
    } > "$IRQ_CONF" 2> /dev/null
}

irq_pin_keyword() {
    local irq prop pinned
    pinned="$(getprop vtools.scene.irq.pinned)"
    for irq in $(irq_numbers_for "$1"); do
        case "$irq" in '' | *[!0-9]*) continue ;; esac
        [[ -e "/proc/irq/$irq/smp_affinity_list" ]] || continue
        prop="vtools.scene.irq.bak.$irq"
        [[ -z "$(getprop $prop)" ]] && setprop $prop "$(read_val "/proc/irq/$irq/smp_affinity_list")"
        write_val "/proc/irq/$irq/smp_affinity_list" "$2"
        case " $pinned " in
            *" $irq "*) ;;
            *) pinned="$pinned $irq" ;;
        esac
    done
    setprop vtools.scene.irq.pinned "${pinned# }"
}

irq_restore_pins() {
    local irq prop
    for irq in $(getprop vtools.scene.irq.pinned); do
        case "$irq" in '' | *[!0-9]*) continue ;; esac
        prop="$(getprop "vtools.scene.irq.bak.$irq")"
        [[ -n "$prop" ]] && write_val "/proc/irq/$irq/smp_affinity_list" "$prop"
        setprop "vtools.scene.irq.bak.$irq" ""
    done
    setprop vtools.scene.irq.pinned ""
}

apply_irq_balance() {
    local pid
    irq_write_conf
    # Do not fight an instance someone else started (MIUI never does).
    if [[ -z "$(pidof msm_irqbalance)" ]] && [[ -x /vendor/bin/msm_irqbalance ]]; then
        nohup /vendor/bin/msm_irqbalance -f "$IRQ_CONF" > /dev/null 2>&1 &
        setprop vtools.scene.irqbal.owned 1
    fi
    renice -n -10 -p "$(pidof msm_irqbalance)" > /dev/null 2>&1
    irq_pin_keyword "$IRQ_KGSL_PATTERN" "$IRQ_KGSL_AFFINITY"
    irq_pin_keyword "$IRQ_DRM_PATTERN" "$IRQ_DRM_AFFINITY"
}

restore_irq_balance() {
    local pid
    irq_restore_pins
    if [[ "$(getprop vtools.scene.irqbal.owned)" = "1" ]]; then
        for pid in $(pidof msm_irqbalance); do
            # Only stop the instance Scene started with its own conf.
            if grep -q "$IRQ_CONF" "/proc/$pid/cmdline" 2> /dev/null; then
                kill "$pid" 2> /dev/null
            fi
        done
